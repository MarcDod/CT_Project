...
public class HomeScreenResourceManagerActivity extends HomeScreenActivity {
    ...
    public HomeScreenResourceManagerActivity(ActionListener groceryListListener,
            ActionListener showOrders, HomeManagerResourceManager homeManager) {
        ...
        this.buttons[0].setImage(drawGroceryList(this.buttons[0].getWidth(),
                this.buttons[0].getHeight(), Color.decode("0x1fbc00"), Color
                .decode("0x0a3d00"), NEW_ORDERS, homeManager
                .getNotWatchedOrdes()));
        // ActionListener müssen in die richtige Reihenfolge gebracht werdern
        ...
        this.buttons[1].setImage(drawGroceryList(this.buttons[1].getWidth(),
                this.buttons[1].getHeight(), Color.decode("0x005fe2"), Color
                .decode("0x002c68"), "EINKAUFSLISTEN", homeManager
                .getGrocerySize()));
        this.buttons[1].addActionListener(groceryListListener);
    }

    private BufferedImage drawGroceryList(int width, int height,
            Color beginColor, Color endColor, String text, int number) {
        ...
        g2d.drawString(numberString, width / 4 - fm.stringWidth(numberString)
                / 2, height / 2 + fm.getHeight() / 2);

        GradientPaint gradient = new GradientPaint(0, 0, beginColor, width, 0,
                endColor);
