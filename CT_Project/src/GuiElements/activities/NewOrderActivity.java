...
public class NewOrderActivity extends Activity {
    ...
    public NewOrderActivity(NewOrderManager newOrderManager, ActionListener newOrderListener) {
        ...
        this.itemSelector = new ItemSelector((int)(componentWidth * 0.9), 300, newOrderManager.getItems(), new ArrayList<>());
        this.itemSelector.setLocation(componentX + (componentWidth - this.itemSelector.getWidth()) / 2, newItem.getY() + newItem.getHeight());
...
//</editor-fold> 
        ...
        this.createItemButton = initButton(componentWidth - newItem.getWidth(), 50,
                this.newItem.getX() + this.newItem.getWidth(),
                this.newItem.getY(), "HINZUFÜGEN");
        this.createItemButton.setFont(new Font(Gui.BUTTON_FONT.getName(), Font.BOLD, 20));
        ...
        this.number.setLocation(this.itemSelector.getX(), this.itemSelector.getY() + this.itemSelector.getHeight() + 30);
        
        
        this.date = new DateSelector(this.itemSelector.getWidth() - this.number.getWidth() - 30, 90);
        this.date.setLocation(this.number.getX() + this.number.getWidth() + 30, this.number.getY() + (this.number.getHeight() - this.date.getHeight()) / 2);
        
        this.deadLine = new JLabel();
        this.deadLine.setSize(date.getWidth(), 25);
        this.deadLine.setLocation(date.getX(), date.getY() - deadLine.getHeight());
        this.deadLine.setFont(new Font(Gui.BUTTON_FONT.getName(), Font.PLAIN, 20));
    ...
    private void makeNewOrder(ActionEvent ae) {
        try {
            ...
            if(number == 0) throw new IllegalArgumentException("Mindestens ein Item muss bestellt werden");
